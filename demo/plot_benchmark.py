...
parser.add_argument(
    "--acquisition-fn", type=str, default="ei", help="Acquisition function name."
)
...
df["Top N Variants"] = pd.Categorical(
    df["Top N Variants"], categories=top_n_variants, ordered=True
)
...
g = sns.FacetGrid(
    df, col="Sample Size", col_wrap=2, height=5.5, aspect=1.8, sharey=True
)  # Adjusted height and aspect
...
def plot_box_and_strip(data, x, y, **kwargs):
    ...
    for i, median in enumerate(medians):
        ax.text(
            i, median + 1, f"{median:.2f}", ha="center", va="bottom", color="black"
        )  # Adjusting the position to be just above the median line
...
g.add_legend(
    title="Dataset", bbox_to_anchor=(0.87, 0.47), loc="center left", borderaxespad=0
)

# Set the main title and subtitle
g.fig.suptitle(
    "Rounds to Discover Top N Variants Across Different Sample Sizes", y=0.94, x=0.44
)
plt.text(
    0.44,
    0.9,
...
# Save the plot
plt.savefig(
    f"usrs/benchmark/first_discovered_{MODEL}_{REP}_{ACQ_FN}.png",
    bbox_inches="tight",
    dpi=300,
)
