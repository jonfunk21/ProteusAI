...
parser.add_argument(
    "--user", type=str, default="benchmark", help="User name or identifier."
)
parser.add_argument(
    "--sample-sizes",
    type=int,
...
parser.add_argument(
    "--zs-model", type=str, default="esm1v", help="Zero-shot model name."
)
parser.add_argument(
    "--benchmark-folder",
    type=str,
    default="demo/demo_data/DMS/",
...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    default=[5, 10, 20, 50, "improved"],
    help="List of improvements.",
)
parser.add_argument(
    "--acquisition_fn", type=str, default="ei", help="ProteusAI acquisition functions"
)
parser.add_argument(
    "--k_folds", type=int, default=5, help="K-fold cross validation for sk_learn models"
)
...
def benchmark(dataset, fasta, model, embedding, name, sample_size, results_df):
    ...
    model.train(
        ...
        x=REP,
        split={
            "train": zs_selected[:n_train],
            "test": zs_selected[n_train : n_train + n_test],
            "val": zs_selected[n_train + n_test : _sample_size],
        },
        seed=SEED,
        model_type=MODEL,
        k_folds=K_FOLD,
    ...
    # add to results df
    results_df = add_to_data(
        data=results_df,
        proteins=zs_selected,
        iteration=iteration,
    ...
    actual_top_variants = {
        count: [prot.name for prot in actual_top_variants[:count]]
        for count in top_variants_counts
    ...
    actual_top_variants["improved"] = [
        prot.name for prot in lib.proteins if prot.y > y_mean + 1 * y_std
    ]
    ...
    while len(ranked_search_space) >= sample_size:
        ...
        for c, count in enumerate(found_counts):
            found = len(
                [
                    prot.name
                    for prot in sampled_data
                    if prot.name in actual_top_variants[count]
                ]
            )
        ...
        model.train(
            library=lib, x=REP, split=split, seed=SEED, model_type=MODEL, k_folds=K_FOLD
        )
        ...
        (
            ...
            y_val,
            sorted_acq_score,
        ) = model.predict(ranked_search_space, acq_fn=ACQ_FN)
...
for i in range(len(datasets)):
    for N in SAMPLE_SIZES:
        print(
            f"RUNNING model:{MODEL}, rep:{REP}, acq:{ACQ_FN}, sample_size:{N}",
            flush=True,
        )
        ...
        found_counts, results_df = benchmark(
            d,
            f,
            model=MODEL,
            embedding=REP,
        ...
        with open(
            os.path.join(
                "usrs/benchmark/", f"first_discovered_data_{MODEL}_{REP}_{ACQ_FN}.json"
            ),
            "w",
        ) as file:
        ...
        results_df.to_csv(
            os.path.join("usrs/benchmark/", f"results_df_{MODEL}_{REP}_{ACQ_FN}.csv"),
            index=False,
        )
